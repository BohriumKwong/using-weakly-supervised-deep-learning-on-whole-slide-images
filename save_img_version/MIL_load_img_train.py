...
#    fpr = float(np.logical_and(pred==1,neq).sum())/(real==0).sum()
    fnr = float(np.logical_and(pred==0,neq).sum())/(real==1).sum() if (real==1).sum() >0 else 0.0
...
class MILdataset(data.Dataset):
    def __init__(self, libraryfile='', k=0, transform=None,image_save_dir=''):
...
#                                                    self.patch_size[slideIDX])).convert('RGB')
            img = Image.open(os.path.join(self.image_save_dir,str(self.targets[slideIDX]),self.slidenames[slideIDX] + '_' + str(k) + '_' + str(j)+'.jpg'))
#            if img.size != (224,224):
#                img = img.resize((224,224),Image.BILINEAR)
            ...
            slideIDX, (k,j), target,h_value = self.t_data[index]
            img = Image.open(os.path.join(self.image_save_dir,str(self.targets[slideIDX]),self.slidenames[slideIDX] + '_' + str(k) + '_' + str(j)+'.jpg'))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='MIL-nature-medicine-2019 tile classifier training script')
    parser.add_argument('--train_lib', type=str, default='output/lib/512/tum_region/cnn_train_data_lib.db', help='path to train MIL library binary')
    parser.add_argument('--val_lib', type=str, default='output/lib/512/tum_region/cnn_val_data_lib.db', help='path to validation MIL library binary. If present.')
    parser.add_argument('--train_dir',type=str, default='/cptjack/totem_other/totem/weakly_supervised_MIL_202005/train')
    parser.add_argument('--val_dir',type=str, default='/cptjack/totem_other/totem/weakly_supervised_MIL_202005/val')
