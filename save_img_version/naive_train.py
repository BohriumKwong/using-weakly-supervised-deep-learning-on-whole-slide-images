...
#    normalize = transforms.Normalize(mean=[0.736, 0.58, 0.701],std=[0.126, 0.144, 0.113])
    ...
    #load data
    train_dset = MILdataset(args.train_lib, 0,train_trans,args.train_dir,target_train_slide,)
...
#    time_mark = '2020_03_06_'
    ...
    #open output file
    fconv = open(os.path.join(args.output, time_mark + 'CNN_convergence_224_resnet18_CE.csv'), 'w')
...
#                                  }
    ...
    for epoch in range(args.nepochs):
        ...
        train_whole_precision,train_whole_recall,train_whole_f1,train_whole_loss = train_predict(epoch, train_loader, model, criterion, optimizer,'train',True,2)
...
#        v_pred = group_max(val_probs,v_topk,args.k)
        ...
        fconv = open(os.path.join(args.output, time_mark + 'CNN_convergence_224_resnet18_CE.csv'), 'a')
        fconv.write(result)
        fconv.close()
        #Save best model
...
#        pickle.dump(topk_list, fp)
        ...
        torch.save(val_probs_save, os.path.join(args.output,'numpy_save/final/minmax',time_mark +'resnet18_val_probs_224.db'))
        torch.save(eopch_save, os.path.join(args.output, time_mark +'resnet18_ibn_b_checkpoint_224.pth'))
...
def train_predict(run, loader, model, criterion, optimizer,phase='train',grad_add = False,accumulation = 2):
    ...
    if phase == 'train':
        return round(whole_metrics_meters['precision'],3),round(whole_metrics_meters['recall'],3),round(whole_metrics_meters['f1score'],3),round(whole_loss/(i+1),3)
    else:
        probs = np.delete(probs, 0, axis=0)
        return round(whole_metrics_meters['precision'],3),round(whole_metrics_meters['recall'],3),round(whole_metrics_meters['f1score'],3),probs.reshape(-1,2)
...
#            g = random.sample(g, int(len(g)/3))
            #样本过多的时候,可以通过上述语句进行下采样到原来的1/3或者你指定的比例. by Bohrium.Kwong 2020.05.19
            grid.extend(g)    
...
#        self.level = lib['level']
    ...
    def maketraindata(self, phase = 'train',idxs = None,repeat=0):
        if idxs is None:
        #idxs为None的情况专门给naive_train使用，都是直接遍历数据集的所有grid生成对应的t_data
            if phase == 'train':
                #如果是train的情况返回的h_value就由随机生成
                self.t_data = [(self.batch[self.slideIDX[x]],self.slideIDX[x],self.grid[x],self.targets[self.slideIDX[x]],\
                                random.uniform(-0.12,0.12)) for x in range(len(self.grid))]
            elif phase == 'val':
                self.t_data = [(self.batch[self.slideIDX[x]],self.slideIDX[x],self.grid[x],self.targets[self.slideIDX[x]],\
                                0) for x in range(len(self.grid))]        
        else:
        #else分支为原来MIL过程的maketraindata,但不会在naive_train中使用. by Bohrium.Kwong 2020.05.15
            ...
            if abs(repeat) == 0:
                ...
                for y in range(-100,int(100 + repeat/2),int(100*2/repeat)):
                    ...
                    self.t_data = self.t_data + [(self.batch[self.slideIDX[x]],self.slideIDX[x],self.grid[x],self.targets[self.slideIDX[x]],y/1000) \
                                                 for x in idxs]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='MIL-nature-medicine-2019 tile classifier training script')
    parser.add_argument('--train_lib', type=str, default='train_MIL_library_binary_file_path', help='path to train MIL library binary')
    parser.add_argument('--select_lib', type=str, default='validation MIL library binary_file_path', help='path to validation MIL library binary. If present.')
    parser.add_argument('--train_dir',type=str, default='train_dataset_dir', help='root path to whole dataset. If present.')
