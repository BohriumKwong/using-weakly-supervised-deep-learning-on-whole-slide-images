...
#    get_feature_model = nn.Sequential(*list(list(model.children())[0].children())[:-2]).cuda()
    ...
    lstm_model = ori_lstm(512,16,2,True,2).cuda()
...
#    time_mark = '2020_03_06_'
    ...
    #open output file
    fconv = open(os.path.join(args.output, time_mark + 'Conv_LSTM_512.csv'), 'w')
...
#    t_pred = group_max(train_probs,topk,args.k)
    ...
    val_dset.maketraindata(v_topk,0)
    val_dset.setmode(2)
...
#                early_stop_count +=1              
        ...
        whole_acc,whole_recall,whole_fnr,whole_loss = train(epoch, train_loader, get_feature_model,lstm_model, criterion, optimizer)
        print('\tTraining  Epoch: [{}/{}] Acc: {} Recall:{} Fnr:{} Loss: {}'.format(epoch+1, \
              args.nepochs, whole_acc,whole_recall,whole_fnr,whole_loss))
        
        train_dset.maketraindata(topk,0)
        train_dset.setmode(2)
        tmp_train_probs = inference(epoch, train_loader, get_feature_model,lstm_model, args.batch_size, 'train')         
...
#        np.save('output/numpy_save/val_infer_probs.npy',pred)
        tmp_val_probs = inference(epoch, val_loader, get_feature_model,lstm_model, args.batch_size, 'val')
...
def inference(run, loader, get_feature_model, model, batch_size,phase):
    model.eval()
    probs = np.zeros((1,2))
#    logs = {}
    whole_probably = 0.
...
#                output = F.softmax(model(input), dim=1)
                output = F.softmax(model(features.view(-1,1,512)), dim=1)
...
def train(run, loader,get_feature_model, model, criterion, optimizer):
    model.train()
    whole_loss = 0.
    whole_acc = 0.
    whole_recall = 0.
    ...
    with tqdm(loader, desc = 'Epoch:' + str(run+1) + ' is trainng', \
        ...
        for i, (input, target) in enumerate(iterator):
            ...
            target = target.cuda()
            features = get_feature_model(input)
            output = F.softmax(model(features.view(-1,1,512)))
...
#    fpr = float(np.logical_and(pred==1,neq).sum())/(real==0).sum()
    fnr = float(np.logical_and(pred==0,neq).sum())/(real==1).sum() if (real==1).sum() >0 else 1    
